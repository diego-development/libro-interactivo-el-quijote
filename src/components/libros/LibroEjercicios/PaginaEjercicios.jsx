  ...
  return (
    <div ref={ref} className="page relative overflow-hidden font-[Caveat]">
      ...
      <div className="relative z-10 flex flex-col items-center justify-between h-full w-full"
           style={{ padding: isSmall ? '4% 6%' : '5% 7%', gap: isSmall ? '0.6rem' : '2rem' }}>
        {grupo.map((q, i) => (
          <motion.div key={q.id || i} className="ejercicio-card w-full p-3 md:p-5 lg:p-6"
            ...
            <div className="flex flex-col gap-1.5 w-full">
              {q.answers.map((a, idx) => {
                ...
                return (
                  <button key={idx} onClick={() => seleccionar(q.id, idx, tipo)}
            ...
            {verificado && <p className="feedback mt-2 text-center italic bg-white/60 p-2 rounded-md">{q.feedback}</p>}
          </motion.div>
        ))}

        {!verificado && grupo.length > 0 && (
          <div className="flex justify-center mt-3 mb-2">
            <button onClick={comprobar} className="btn-comprobar">Comprobar</button>
          </div>
        )}
      </div>
    </div>
